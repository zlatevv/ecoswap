        ...
        <div className="bg-white rounded-2xl border border-stone-200 shadow-sm p-5 flex flex-col gap-4">
            ...
            <div className="flex items-center gap-3">
                <div className="flex-1 bg-stone-50 rounded-xl p-3 border border-stone-100">
                    <p className="text-xs text-stone-400 font-semibold mb-1 uppercase tracking-wide">
            ...
            <div className="flex items-center justify-between">
                <div className="text-xs text-stone-400">
                    {isIncoming
                        ? <>From <strong className="text-stone-600">{swap.requester?.username}</strong></>
                        : <>To <strong className="text-stone-600">{swap.requestedProduct?.user?.username}</strong></>
                    }
                </div>
            ...
            {swap.message && (
                <div className="bg-stone-50 rounded-xl px-4 py-3 border border-stone-100">
                    ...
                    <p className="text-sm text-stone-600 italic">"{swap.message}"</p>
                </div>
            )}
            ...
            {isIncoming && swap.status === 'PENDING' && (
                <div className="flex gap-3 pt-1">
                    <button
                        ...
                        disabled={loading}
                        className="flex-1 py-2 border border-rose-200 text-rose-600 rounded-xl text-sm font-semibold hover:bg-rose-50 transition-colors disabled:opacity-60 flex items-center justify-center gap-2"
                    >
                    ...
                    <button
                        ...
                        disabled={loading}
                        className="flex-1 py-2 bg-emerald-600 text-white rounded-xl text-sm font-semibold hover:bg-emerald-700 transition-colors disabled:opacity-60 flex items-center justify-center gap-2"
                    >
...
export default function SwapRequests({ userId }) {
    ...
    return (
        <div className="flex-1 overflow-auto p-8">
            <div className="mb-8">
                <h1 className="text-3xl font-bold text-stone-900 tracking-tight">Swap Requests</h1>
                ...
                {['incoming', 'outgoing'].map(t => (
                    ...
                    >
                        ...
                        <span className={`ml-2 text-xs px-1.5 py-0.5 rounded-md ${
                            tab === t ? 'bg-emerald-100 text-emerald-700' : 'bg-stone-200 text-stone-500'
                        }`}>
            ...
            {!loading && current.length === 0 && (
                <div className="text-center py-16">
                    <RefreshCw className="h-10 w-10 text-stone-200 mx-auto mb-3" />
                    <p className="text-stone-400 text-sm">No {tab} swap requests yet.</p>
                </div>
            )}
